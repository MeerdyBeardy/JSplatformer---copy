function Level(plan) {
	// Use length of a single row to set the width of the level
	this.width = plan[0].lenth;
	//Use the number of rows to set the hieght
	
	this.height = plan.length;
	
	//Store the indicidual tiles in our own, separate array
	this.grid = [];
	...
	for (var y = 0; y < this.height; y++) {
		...
		for (var x = 0; x < this.width; x++) {
			...
			if (ch==='@')
				...
				fieldType = "wall";
				// Because there is a thrid case (space ' '), use an "else if" instead of "else"
			else if (ch === '!')
				fieldType = "lava";
...
DOMDisplay.prototype.drawBackground = function() {
	...
	table.style.width = this.lecel.width * scale + "px";
...
DOMDisplay.prototype.scrollPlayerIntoView = function() {
	...
	var hieght = this.wrap.clientHeight;
...
//Organize a single and begin animation
...
function runGame(plans, Display) {
	function startLevel(n) {
		...
		//pass ina reference to Display function, DOMDisplay (in index.html).
